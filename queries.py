...
def Q_4(conn, execution_time):
    ...
    query = """ 
SELECT t.team_name, COUNT(p.event_id) AS passes_made
FROM Team t
JOIN Pass p ON t.team_id = p.team_id
...
def Q_5(conn, execution_time):
    ...
    query = """ 
SELECT p.player_name, COUNT(pa.event_id) AS passes_received
FROM Player p
JOIN Pass pa ON p.player_id = pa.recipient_id
JOIN Match m ON pa.match_id = m.match_id
JOIN Season s ON m.season_id = s.season_id
JOIN Competition c ON m.competition_id = c.competition_id
...
def Q_6(conn, execution_time):
    ...
    query = """ 
SELECT Team.team_name, COUNT(Events.event_id) AS shots_made
FROM Team
JOIN Events ON Team.team_id = Events.team_id
JOIN Match ON Events.match_id = Match.match_id
...
AND Events.type_id = (SELECT type_id FROM type WHERE type_name = 'Shot')
...
JOIN Competition c ON m.competition_id = c.competition_id
WHERE m.season_id = (SELECT season_id FROM Season WHERE season_name = '2020/2021')
AND pa.technique_id = (SELECT technique_id FROM technique WHERE technique_name = 'Through Ball')
AND c.competition_name = 'La Liga'
GROUP BY p.player_name
HAVING COUNT(*) >= 1
...
def Q_9(conn, execution_time):
    ...
    query = """ 
SELECT p.player_name, COUNT(*) AS successful_dribbles_count
FROM Events e
...
WHERE m.season_id IN (
    SELECT season_id 
    FROM Season 
    WHERE season_name IN ('2020/2021', '2019/2020', '2018/2019')
)
AND d.outcome_id = (SELECT outcome_id FROM outcome WHERE outcome_name = 'Complete')
AND c.competition_name = 'La Liga'
GROUP BY p.player_name
HAVING COUNT(*) >= 1
ORDER BY successful_dribbles_count DESC;
    """
...
def Q_10(conn, execution_time):
    ...
    query = """ 
SELECT p.player_name, COUNT(*) AS dribbles_past_count
FROM Events e
JOIN Dribbled_Past dp ON e.event_id = dp.event_id
JOIN Player p ON e.player_id = p.player_id
JOIN Match m ON e.match_id = m.match_id
JOIN Competition c ON m.competition_id = c.competition_id
WHERE m.season_id = (SELECT season_id FROM Season WHERE season_name = '2020/2021')
...
GROUP BY p.player_name
HAVING COUNT(*) >= 1
ORDER BY dribbles_past_count ASC;
    """
